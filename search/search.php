...
function getMainCatList()
{
    ...
    $cat_name = isset($_POST['cat_name']) ? mysqli_real_escape_string($cnn, $_POST['cat_name']) : '';



    // Check if the search keyword is empty
    if (empty($cat_name)) {
        error422('Please enter Category Name LIKE cat_name'); // Call error422 function with the new message
    }
    ...
    $query = "SELECT p.name,p.image,p.price,p.des,p.status,p.id,c.name as cat_name,s.name as sub_name FROM pets AS p JOIN category AS c ON p.cat_id=c.id JOIN subcategory AS s ON p.sub_id=s.id WHERE c.name LIKE '%$cat_name%' AND p.status='Active' ORDER BY p.id DESC";
