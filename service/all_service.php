...
function getMainCatList()
{
    ...
    $query = "SELECT id,name,image,des,price FROM service WHERE status='Active' ORDER BY id DESC";
    ...
    if ($query_run) {
        if (mysqli_num_rows($query_run) > 0) {
            ...
            foreach ($res as &$pets) {
               ...
               $wishlist_query = mysqli_query($cnn, "SELECT * FROM service_wishlist WHERE s_id = '{$pets['id']}' AND u_id = '$user_id'");
            ...
            header("HTTP/1.0 404 No Service Found");
            return json_encode($data);
        }
    } else {
        $data = [
