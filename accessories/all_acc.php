...
function getMainCatList()
{
    ...
    $query = "SELECT c.name AS cat_name,p.name AS pet_category,a.* FROM accessories AS a JOIN acce_catgeory AS c ON a.cat_id=c.id JOIN category AS p ON a.pet_cat=p.id WHERE a.status='Active' ORDER BY a.id DESC";
    $query_run = mysqli_query($cnn, $query);

    if ($query_run) {
        if (mysqli_num_rows($query_run) > 0) {
            $res = mysqli_fetch_all($query_run, MYSQLI_ASSOC);
            foreach ($res as &$accessories) {
                // Check if the pet is in the user's wishlist
               $wishlist_query = mysqli_query($cnn, "SELECT * FROM acc_wishlist WHERE a_id = '{$accessories['id']}' AND u_id = '$user_id'");
                $is_wishlisted = mysqli_num_rows($wishlist_query) > 0; // True if the pet is in the wishlist
               // Set wishlist status
               $accessories['wishlist'] = $is_wishlisted ? 1 : 0;
           }
            $data = [
                'success' => true,
