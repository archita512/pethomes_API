...
function getMainCatList()
{
    ...
    $query = "SELECT p.name,p.image,p.price,p.des,p.status,p.id,c.name as cat_name,s.name as sub_name FROM pets AS p JOIN category AS c ON p.cat_id=c.id JOIN subcategory AS s ON p.sub_id=s.id WHERE p.cat_id='$cat_id' AND p.status='Active' ORDER BY p.id DESC";
